...
public class GestorPelicula {
    ...
    public void inicializarLista(){
        ...
        listaPeliculas.add(new Pelicula(
                ...
                "George Peque", "Jack Routerman", 2014, Paises.ESTADOS_UNIDOS,
                ClasificacionEdad.ATP, GeneroPelicula.COMEDIA, EstadoPelicula.NO_DISPONIBLE
        ));
        ...
        listaPeliculas.add(new Pelicula(
                "Promoción 2024: ¡Nos vemos en verano!", Idioma.ITALIANO, Idioma.ESPAÑOL, Duration.ofMinutes(134),
                "Ludricia Mártir", "Adrian Rock", 2018, Paises.ITALIA,
                ClasificacionEdad.MAS_16, GeneroPelicula.SUSPENSO, EstadoPelicula.NO_DISPONIBLE
        ));

        listaPeliculas.add(new Pelicula(
                "Henry Patter y el horno microondas", Idioma.INGLES, Idioma.ESPAÑOL, Duration.ofMinutes(102),
    ...
    public void agregarPelicula(){
        ...
        while(datosValidos==0){ //Ingresar el titulo
            try{
                ...
                pelicula.setTitulo(new GestorDatos().ingresarTexto(entrada));
                datosValidos++;
        ...
        while(datosValidos==1){ //Seleccionar el idioma
            try{
                ...
                pelicula.setIdioma(new GestorEnums<>(Idioma.class).seleccionarElemento(entrada));
                datosValidos++;
        ...
        while(datosValidos==2){ //Seleccionar el idioma de los subtitulos
            try{
                ...
                pelicula.setIdiomaSubtitulos(new GestorEnums<>(Idioma.class).seleccionarElemento(entrada));
                datosValidos++;
            }catch (NumeroFueraDelRangoException e){
                System.out.println(e.getMessage());
            }catch (SoloNumerosException e){
        ...
        while(datosValidos==3){ //Ingresar la duracion
            try{
                pelicula.setDuracion(new GestorDatos().ingresarDuracion(entrada));
                datosValidos++;
            }catch (NumeroFueraDelRangoException e){
                System.out.println(e.getMessage());
            }catch (SoloNumerosException e){
        ...
        while(datosValidos==4){ //Ingresar el productor
            try{
                ...
                pelicula.setProductor(new GestorDatos().ingresarNombreCompleto(entrada));
                datosValidos++;
            }catch (StringEnBlancoException e){
                System.out.println(e.getMessage());
            }
        ...
        while(datosValidos==5){ //Ingresar el director
            try{
                ...
                pelicula.setDirector(new GestorDatos().ingresarNombreCompleto(entrada));
                datosValidos++;
            }catch (StringEnBlancoException e){
                System.out.println(e.getMessage());
            }
        ...
        while(datosValidos==6){ //Ingresar el año
            try{
                pelicula.setAñoLanzamiento(new GestorDatos().ingresarAño(entrada));
                datosValidos++;
            }catch (NumeroFueraDelRangoException e){
                System.out.println(e.getMessage());
            }catch (SoloNumerosException e){
        ...
        while(datosValidos==7){ //Ingresar el pais
            try{
                ...
                pelicula.setPais(new GestorEnums<>(Paises.class).seleccionarElemento(entrada));
                datosValidos++;
            }catch (NumeroFueraDelRangoException e){
                System.out.println(e.getMessage());
            }catch (SoloNumerosException e){
        ...
        while(datosValidos==8){ //Ingresar la clasificacion por Edad
            try{
                ...
                pelicula.setClasificacionEdad(new GestorEnums<>(ClasificacionEdad.class).seleccionarElemento(entrada));
                datosValidos++;
            }catch (NumeroFueraDelRangoException e){
                System.out.println(e.getMessage());
            }catch (SoloNumerosException e){
        ...
        while(datosValidos==9){ //Ingresar el generoPelicula
            try{
                ...
                pelicula.setGeneroPelicula(new GestorEnums<>(GeneroPelicula.class).seleccionarElemento(entrada));
                datosValidos++;
            }catch (NumeroFueraDelRangoException e){
                System.out.println(e.getMessage());
            }catch (SoloNumerosException e){
        ...
        while(datosValidos==10){ //Ingresar el EstadoPelicula
            try{
                ...
                pelicula.setEstadoPelicula(new GestorEnums<>(EstadoPelicula.class).seleccionarElemento(entrada));
                datosValidos++;
            }catch (NumeroFueraDelRangoException e){
                System.out.println(e.getMessage());
            }catch (SoloNumerosException e){
        ...
        System.out.println("Estos son los datos ingresados." + pelicula);

        if (new GestorDatos().confirmacion(entrada, " Desea guardar estos datos?")){
            listaPeliculas.add(pelicula);
        }else { //Agregar otro else if que permita elegir entre modificar o eliminar la peli
            modificarPelicula(entrada, pelicula);
        }
    }
    ...
    public void modificarPelicula(Scanner entrada, Pelicula pelicula) {
        int opcion = 0;
        ...
        while (salirSwitch) {
                ...
                if (opcion >= 1 && opcion <= 11) {
                    switch (opcion) {
                        case 1://Modificar el titulo
                            do {
                                try {
                                    ...
                                    pelicula.setTitulo(datos.ingresarTexto(entrada));
                                    salirBucle = false;
                                } catch (StringEnBlancoException e) {
                                    System.out.println(e.getMessage());
                                }
                        ...
                        case 4: //Cambiar duracion
                            do {
                                try {
                                    pelicula.setDuracion(datos.ingresarDuracion(entrada));
                                    entrada.nextLine();
                                    salirBucle = false;
                                } catch (StringEnBlancoException e) {
                                    System.out.println(e.getMessage());
                        ...
                        case 5: //Cambiar Productor
                            do {
                                try {
                                    System.out.println("Productor:");
                                    pelicula.setProductor(datos.ingresarNombreCompleto(entrada));
                                    salirBucle = false;
                                } catch (StringEnBlancoException e) {
                                    System.out.println(e.getMessage());
                                }
                        ...
                        case 6: //Cambiar Director
                            do {
                                try {
                                    System.out.println("Director:");
                                    pelicula.setDirector(datos.ingresarNombreCompleto(entrada));
                                    salirBucle = false;
                                } catch (StringEnBlancoException e) {
                                    System.out.println(e.getMessage());
                                }
                        ...
                        case 7: //Cambiar año
                            do {
                                try {
                                    pelicula.setAñoLanzamiento(datos.ingresarAño(entrada));
                                    salirBucle = false;
                                } catch (StringEnBlancoException e) {
                                    System.out.println(e.getMessage());
                                }
            ...
            } catch (NumeroFueraDelRangoException | SeleccionInvalidaException | StringEnBlancoException |
                     SoloNumerosException e) {
                System.out.println(e.getMessage());
            }
        }
    }
    ...
    public ArrayList<Pelicula> retornarCartelera(){
        ArrayList<Pelicula> cartelera = new ArrayList<>();
        for (Pelicula pelicula : listaPeliculas){
            if (pelicula.getEstadoPelicula() == EstadoPelicula.EN_CARTELERA){
                cartelera.add(pelicula);
            }
        } //Habria que agregarle un contador con el numero de id para seleccionarlo
        return cartelera;
    }
